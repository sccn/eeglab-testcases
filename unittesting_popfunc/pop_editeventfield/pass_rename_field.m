...
function passed = pass_rename_field()


		eeg = pop_loadset('test.set','');
		rename = 'position->place';		% to be modified

		[neweeg com] = pop_editeventfield(eeg,'rename',rename);
		if isfield(neweeg.event,'position') ...
				|| ~isfield(neweeg.event,'place') ...
				|| ~isequal([neweeg.event.place],[eeg.event.position])
