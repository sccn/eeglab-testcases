...
function passed = pass_rename_field_not_exist()

		eeg = pop_loadset('test.set','');
		rename = 'abcd->place';		% to be modified

		[neweeg com] = pop_editeventfield(eeg,'rename',rename);		% expected no changes since field 'abcd' does not exist
		if isfield(neweeg.event,'place') ...
