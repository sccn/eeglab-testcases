...
function pass_add_field_vector_indices()

		eeg = pop_loadset('test_no_events.set','');
		test = [-7 2 19.5];		% to be added

		[neweeg com] = pop_editeventfield(eeg,'test',test);
        tmpevent = neweeg.event;
		if ~isfield(neweeg.event,'test') || ~isequal([tmpevent.test],test)
