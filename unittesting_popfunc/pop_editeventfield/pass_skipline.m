...
function passed = pass_skipline()


		eeg = pop_loadset('test_no_events.set','');
		test = 'skipline.txt';		% to be added
		skipline = 3;		% skip 3 first lines from file
		testvar = [0 -5.33 14.7 10];

		[neweeg com] = pop_editeventfield(eeg,'test',test,'skipline',skipline);
        tmpevent = neweeg.event;
		if ~isfield(neweeg.event,'test') || ~isequal([tmpevent.test],testvar)
