...
function passed = pass_modify_field_indices()

		eeg = pop_loadset('test_no_events.set','');
		type = 54321;		% to be modified
		indices = 2;

		[neweeg com] = pop_editeventfield(eeg,'type',type,'indices',indices);
		if ~isequal(neweeg.event(2).type,type)
