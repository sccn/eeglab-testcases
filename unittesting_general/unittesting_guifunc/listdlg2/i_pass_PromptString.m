...
% function passed = i_pass_PromptString()
%     passed = tc_notpassed;
%     try
% 	ListString = {'This', 'is', 'a', 'testcase'};
% 	PromptString = {'i', 'pass', 'PromptString'};
% 	[rselection, rok] = listdlg2('ListString', ListString, 'PromptString', PromptString);
% 
% 	if isempty(rselection) && near(rok, 0) % cancel
% 	    passed = tc_passed;
% 	elseif ~isempty(rselection) && near(rok, 1) % ok & items chosen
% 	    [b,num] = unique(rselection);
% 	    if min(rselection) >= 1 ...
% 	    && max(rselection) <= 4 ...
% 	    && near(b, rselection(num))
% 		passed = tc_passed;
% 	    end;
